...
class PeopleToolbar(CMSToolbar):
    ...
    def populate(self):
        ...
        if user and view_name:
            ...
            group = person = None
            if view_name == 'ssrd_people:group-detail':
                group = get_obj_from_request(Group, self.request)
            elif view_name in [
                    'ssrd_people:person-detail',
                    'ssrd_people:download_vcard']:
                person = get_obj_from_request(Person, self.request)
                if person and person.groups:
                    group = person.primary_group
            elif view_name in ['ssrd_people:group-list', ]:
                pass
            else:
                # We don't appear to be on any ssrd_people views so this
                # menu shouldn't even be here.
                return

            menu = self.toolbar.get_or_create_menu('people-app', "People")
            change_group_perm = user.has_perm('ssrd_people.change_group')
            add_group_perm = user.has_perm('ssrd_people.add_group')
            group_perms = [change_group_perm, add_group_perm]

            change_person_perm = user.has_perm('ssrd_people.change_person')
            add_person_perm = user.has_perm('ssrd_people.add_person')
